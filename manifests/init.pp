...
class node_builder(
  ...
  $ldap_search_base = "ou=users,dc=airgapit,dc=com",
    ...
    exec { "deploy node builder":
      ...
      command  =>  "/bin/rm -rf ${deploy_path}/node_builder* ; sleep 5s ; /usr/bin/curl  --location --referer \";auto\" -o /tmp/node-builder.war \"${artifact_url}\"  ; mv -f /tmp/node-builder.war ${deploy_path} ; mv ${deploy_path}/node-builder-new-version.txt ${deploy_path}/node-builder-version.txt",
      user     => $deploy_user
    }
}
